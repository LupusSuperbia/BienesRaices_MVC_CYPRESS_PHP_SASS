...
describe("Carga la pagina principal", () => {
  it("Prueba el Header de la página principal", () => {
    ...
    // document.querySelector('h1').textContent; es igual a lo de abajo
    // cy.get("h1");

    //.should es para vereficiar si un elemento exista
    cy.get('[data-cy="heading-sitio"]').should("exist");
    cy.get('[data-cy="heading-sitio"]')
      .invoke("text")
      .should("equal", "Venta de casas y Departamentos de Lujo");
    cy.get('[data-cy="heading-sitio"]')
      .invoke("text")
  ...
  it("Prueba el bloque de propiedades", () => {
    ...
    // cy.get('[data-cy="seccion-propiedades"]').should("exist");
    // cy.get('[data-cy="seccion-propiedades"]')
    //   .should("have.prop", "tagName")
    //   .should("equal", "H2");
    // // Propiedades
    // cy.get('[data-cy="contenedor-anuncios"]').should("exist");
    // cy.get('[data-cy="contenedor-anuncios"]')
    //   .find(".anuncio")
    //   .should("have.length", 3);
    // cy.get('[data-cy="contenedor-anuncios"]')
    //   .find(".anuncio")
  ...
  it('Prueba el bloque de Contacto', () => {
    ...
    cy.get('[data-cy="imagen-contacto"]').find('p').invoke('text').should('equal', 'Llena el formulario de conctacto y un asesor se pondría en contacto contigo');
